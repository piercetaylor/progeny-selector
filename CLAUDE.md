...
**M3 is complete.** M3 shipped a two-pass streaming `read_vcf` that fills a preallocated matrix (at 50,000 markers by 2,000 individuals, a 241.0 MiB load peak against a 190.9 MiB matrix; the marker and allele tables come on top of the matrix), sample-chunked `classify`, `rpp` and `ibs_to_sample` that hold analysis memory to 2.52 times the matrix at 50,000 markers by 2,000 individuals (a ratio that depends on shape; docs/limits.md), a measured limits table for CPython and Shinylive (`docs/limits.md`), an accessibility review against WCAG 2.2 AA with axe-core (`docs/accessibility.md`), the three screen-test gaps M1 deferred (a "(no generation)" Navigate choice, a Rank caption naming an unassigned family or generation, and a browser test for the Navigate reload guard), and a command-line BrAPI loader that reads a variant set into the same `Dataset` a file produces. See PLAN.md, "Verification status", for the M1, M2 and M3 verification blocks.
...
Per milestone: `python3 scripts/bench_pipeline.py --markers 50000 --progeny 2000 --out data/bench/50k_x_2000 [--seed 0] [--no-gzip] [--regenerate]`, which measures wall-clock and peak memory through the CPython pipeline on a generated dataset, and `python3 scripts/bench_shinylive.py --site site --case data/bench/6k_x_2000 [--port 8010] [--timeout-min 30]`, which measures the same case in a real browser against an exported Shinylive site. Each prints a Markdown row and writes a JSON file beside the case under the gitignored `data/bench/`; the main session copies the rows into `docs/limits.md`. Nothing else timed may run alongside them.
...
- **`contract/` is the shared input contract.** It is mirrored byte for byte from `backcross`, where the canonical copy lives (docs/adr/0010); change it there first and copy it here. `docs/data-formats.md` keeps criteria.yaml and the outputs, and the two projects are meant to exchange files unchanged. Ask the user before changing any documented column, parameter name or output format, and say which sibling file is affected. The shared contract test suite (`tests/test_contract_cases.py`, `scripts/check_contract.py`) shipped in M2 and runs as a gate in both repos.
...
- Extras are split so the per-commit gates never depend on shinylive or a browser: `dev` (lint, types, unit tests, shiny, pandas), `export` (shinylive and build; shinylive pulls lzstring, which ships only an sdist), `e2e` (pytest-playwright, and the axe bridge `axe-playwright-python` for the accessibility tests). Local venv recipe: `py -3.12 -m venv .venv`, then `.venv\Scripts\python -m pip install -e ".[dev,export,e2e]"` and `.venv\Scripts\python -m playwright install chromium`. Playwright looks for the exact Chromium revision its Python package pins, so a browser installed through npm Playwright is not reused unless the revisions match.
...
- Playwright 1.49 and later launch `chromium-headless-shell` for `headless=True`, and that binary's `performance.measureUserAgentSpecificMemory()` does not work. `scripts/bench_shinylive.py` launches with `channel="chromium"` instead, so the browser memory figures in `docs/limits.md` come from the full browser.
