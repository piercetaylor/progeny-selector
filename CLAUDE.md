...
- **`contract/` is the shared input contract.** It is mirrored byte for byte from `isoline-browser`, where the canonical copy lives (docs/adr/0010); change it there first and copy it here. `docs/data-formats.md` keeps criteria.yaml and the outputs, and the two projects are meant to exchange files unchanged. Ask the user before changing any documented column, parameter name or output format, and say which sibling file is affected. `M3` in both repos calls for contract tests that pass in both.
