...
Status: all nine questions were settled by the maintainer on 2026-09-12. Browser tests run locally as well as in CI; the maintainer approved installing pytest-playwright and Chromium if not already present.
...
# core/strip.py
@dataclass(frozen=True)
class Segment: start: float; end: float; state: int    # fractions of the chromosome length, 0 <= start <= end <= 1; zero width = markers at one position with different states, drawn at least 1 px
@dataclass(frozen=True)
...
def qc_table_rows(qc: list[SampleQC], dataset: Dataset) -> list[dict]
    ...
    # nonparental_rate, expected_rpp, ibs_rp, ibs_donor, flags ('|'-joined), qc_excluded (bool: any flag in QC_EXCLUDING_FLAGS; missing-rate and locus hard filters are shown on Rank)
...
- `test_qc_table.py`: on the fixture result, `qc_table_rows` has 40 rows, `BC2F1-F2-002` has `qc_excluded is True`, `BC2F1-F1-003` has `high_missing` in `flags` and `qc_excluded is False` (high_missing is not a QC-excluding flag; the missing-rate filter excludes it), `uninformative_summary` sums to 25 with `"parents identical (monomorphic)"` first at 20.
...
4. **Shinylive packaging.** SETTLED 2026-09-12 by the maintainer as recommended. Recommendation was: stage a package copy (resolution 5); fall back to a wheel URL on the Pages site only if Pyodide does not import from the staged directory. Grounds: no release and no PyPI dependency needed, and it works from any directory; the URL fallback ties local preview to the internet and to a deploy that has already happened.
...
8. **Shiny version floor.** SETTLED 2026-09-12 by the maintainer as recommended. Recommendation was: keep `shiny>=1.4` and record the versions the laptop and CI actually resolve in the verification block. Grounds: everything used exists at 1.4.0; raising the floor buys nothing verified here.
