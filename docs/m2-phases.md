...
| `CHANGELOG.md` | Changed (with `!`/BREAKING CHANGE footer per CONTRIBUTING): "results.csv schema 1.0.0: missing cells are `NA`, the header is written with no results, new columns `background_model`, `rpp_unit`, `rank_mode`, `assembly`, `results_schema`; `het_rate`, `expected_het` and `expected_rpp` never print `nan`. selected.csv gains `results_schema` and writes `NA` for missing family or generation (docs/adr/0016)." |
