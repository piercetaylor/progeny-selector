...
- Model for next phase: M3 has no plan yet. It starts the way M2 did — a planner (fable) writes `docs/m3-phases.md`, the main session puts its flagged questions to the maintainer before any dispatch, then one implementer per phase with a reviewer on every core, io, model or contract diff. What M2 taught, and what the next planner should be told: demand a discriminating input for every new test, because five tests this milestone passed with their feature deleted and the gates caught none of them; and push before calling a phase finished, because `e2e`, `export` and `r-reader` are CI-only and one of them went red on a runner difference the local run could not reproduce.
