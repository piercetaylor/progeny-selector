...
- Next, in the maintainer's order of choice:
  ...
  2. Real-data acceptance (Q7, `docs/m1-phases.md` question 7): SoyBase Clark isolines (recurrent parent PI548533) on SoySNP50K. Before any download: map each "Clark (n) x donor" cross in PATRIOT's `GS Pedigrees.csv` to an accession PI in GRIN, confirm each is in the SoySNP50K VCF, and settle the Harosoy PI548573 vs PI548531 question. Data goes to the gitignored `data/` folder only. Expect BCnF1 QC flags on every NIL, because they are inbred. Pre-download matching, 2026-09-14 (notes in the gitignored `data/clark-nil-mapping.md`): `GS Pedigrees.csv` is at https://github.com/SoylabSingh/PATRIOT/blob/master/GS%20Pedigrees.csv. It is a 71,650-row genealogy, not a cross list; 167 rows are Clark- or Harosoy-recurrent NILs (n = 2–7) from 39 donors. GRIN confirms PI548533 Clark and PI548573 Harosoy, and the CSV uses PI548573 for Harosoy. PI548531 is 'Chippewa 64', which settles the Harosoy question unless the maintainer disagrees. 18 PIs are confirmed on GRIN, and about 150 NIL PIs are not yet checked. SoySNP50K membership cannot be confirmed without the VCF `#CHROM` header, because SoyBase publishes no separate accession list. No licence page for the SNP data exists; SoyBase asks for citation of Song et al. 2015 and 2016. VCF header check, 2026-09-14: only the header of `glyma.Wm82.gnm2.div.Song_Hyten_2015.vcf.gz` (https://data.soybase.org/Glycine/max/diversity/Wm82.gnm2.div.Song_Hyten_2015/) was streamed, 180 KB of 144 MB, into `data/soysnp50k.gnm2.header.vcf`. The server returns 403 without a browser User-Agent. The header lists 20,087 samples, and all but 152 are PI numbers. The directory README gives `license: Open` and asks for citation of Song et al. 2015 (doi 10.1534/g3.115.019000), which settles the licence question. Clark PI548533, Harosoy PI548573 and PI548531 are all present. Of the 143 NILs named by PI in the pedigree file, 142 are genotyped (73 Clark, 69 Harosoy; mostly n = 5). The per-NIL table is in `data/clark-harosoy-nils-snp50k.csv`. Every PI-form donor checked is present, and so are the GRIN PIs for Higan (PI548342), Chief (PI548314), T117 (PI548169) and Blackhawk (PI548516). Candidate first family: Clark × PI86024, 8 genotyped NILs with the donor genotyped (PI547416, 423, 425, 429, 454, 455, 461, 472). The largest family is Clark × Higan, with 14. Conversion, 2026-09-14: the full VCF was downloaded to `data/`, and its MD5 matches SoyBase's CHECKSUM file (708076e4…). `scripts/soysnp50k_nils.py` streams it and writes one family to `data/nils/<Recurrent>_x_<donor>/` as `genotypes.vcf`, `samples.csv` and `README.md`. The script strips SoyBase's `glyma.Wm82.gnm2.` chromosome prefix, because `normalize_chrom` would otherwise keep those names verbatim and the chromosome-length lookups would miss. It drops scaffold records, and it drops records whose GT allele index exceeds the ALT count; those are malformed in the source (ALT `.` with non-reference calls). Maintainer decisions: generation left empty, run with `exclude_qc_flagged: false`, first family Clark × PI86024. Clark × Higan (`--donor Higan --donor-pi PI548342`) comes second. Clark × PI86024 output: 8 NILs, 41,358 markers, 287 scaffold and 81 malformed records dropped; `load_dataset` loads it (10 samples). Still open before the ranking run: (a) the criteria reader requires at least one target (`model/criteria.py`). Don't use a placeholder. Look up the isoline gene symbol for the eight NILs on GRIN, then the maintainer picks the target locus, with positions on Wm82.a2. (b) Purdy dose `n = 5` is recorded raw, and whether that means BC4 or BC5 is unresolved. (c) Possible M2 contract question, which needs the maintainer and names isoline-browser's copy of `docs/data-formats.md`: should chromosome normalisation accept the SoyBase `glyma.Wm82.gnmN.` prefix? Expected flags on this data: `possible_rp_sample` for NILs with very small introgressions, and `possible_outcross` if the genotyped Clark seed lot differs from the isoline Clark. First ranking run, 2026-09-14: GRIN lists each NIL's genes. e2 is carried by PI547416, 423, 429, 461 and 472; dt1 by PI547429, 454, 455 and 472; Lf1 by PI547425, 455, 461 and 472. Targets are SoyBase Wm82.a2.v1 gene models ±1 Mb, with `rule: any`, `hom_donor`, and `exclude_qc_flagged: false`: E2 Glyma.10g221500 at Gm10:45,294,735–45,316,121; Dt1 Glyma.19g194300 at Gm19:45,183,357–45,185,175; Lf1 Glyma.08g281900 at Gm08:38,660,625–38,661,689. Criteria and results are in `data/nils/Clark_x_PI86024/`. Pass/fail matched GRIN in 31 of 32 calls: e2 6 pass (5 expected), dt1 4/4, Lf1 4/4, all three 1/1 (PI547472). No QC flags fired, and there were no non-parental calls or heterozygotes. The one mismatch is PI547454, GRIN dt1 only: 6 scattered B against 50 A in the e2 window, and 14.3% B genome-wide against 2–5% for the others (Gm14 RPP 0.46). That suggests a sample that is not a clean isoline, which `rule: any` over a wide window lets through. A tighter e2 window (±250 kb, `rule: all`) excludes PI547454 but also fails the GRIN e2 carriers PI547461 and PI547472, so the window is not tuned further. Open: whether the QC should flag a NIL whose donor fraction is far above its family's (no such flag exists), and which window rule the acceptance criteria should use. Second run, 2026-09-14, Clark × Higan (`--donor Higan --donor-pi PI548342`): 14 NILs, all genotyped, same ±1 Mb `rule: any` method. Genes from GRIN: t, l2, Ps-s, pa1, pa2, S, r, Rps1-c. Tested only genes with a SoyBase Wm82.a2 gene model: T Glyma.06g202300 (cloned), R Glyma.09g235100 (cloned), and two GWAS candidates, Ps Glyma.12g187200 and Pa1 Glyma.12g213900. pa2, S, l2 and Rps1-c have no gene model, so they were not tested. Agreement was 44/56. Every GRIN carrier of t (PI547435, 496), r (PI547592) and Ps-s (PI547456, 457, 480, 529) passed with a real donor segment. pa1 passed PI547457, 481 and 482, but PI547634 failed (0 B of 50). The 12 disagreements have three causes. (1) Seven come from 1–6 scattered B calls passing `rule: any`, in PI547457, 592 and 646, the same mechanism as PI547454. (2) Four are real segments that GRIN does not list: PI547634 at T (28 B of 30) and R (10 of 81), and PI547456 in the pa1 window (16 of 50), which is probably linkage drag from its Ps-s segment 2.5 Mb away. (3) One is the missing pa1 call for PI547634. PI547634 is 24.1% donor genome-wide, and PI547646 (Rps1-c) is 8.7%, against 1–5% for the rest, so it is the family's PI547454. `possible_rp_sample` fired on PI547435 and PI547592 (≤1% donor), which is the expected behaviour. Criteria and results are in `data/nils/Clark_x_Higan/`. Correction, 2026-09-15: r on PI547592 is not a real segment. Its calls near R are scattered single B (11 of 81, longest run 2), so under `rule: any` it passed on scattered calls like the others. Window rule decided 2026-09-15 (docs/adr/0011): the maintainer proposed at least 5 donor markers and delegated the design to standard academic practice. `min_markers` counts called markers, not donor markers, so it could not express that. The new target rule `rule: run` passes when a contiguous donor run of at least `min_run` (default 3) calls covers the gene anchor, bridging isolated single mismatches. 5 was rejected because the Lf1 carriers carry only 4 informative donor markers. Rerun through the CLI with `rule: run` (`criteria_*_run.yaml`, `results_*_run.csv`): Clark × PI86024 24/24 (PI547454 now fails e2), Clark × Higan 52/56, total 76/80 against 67/80 under `rule: any`. The four remaining disagreements are PI547634 at T, R and pa1, and PI547592 at R (suspected donor-lot haplotype mismatch). Family-outlier QC decided 2026-09-16 (docs/adr/0012): advisory flag `family_donor_outlier` (96bcde4), which on both families flags exactly PI547454, PI547634 and PI547646.
  3. M2 planning with the planner agent (PLAN.md "Milestones", M2 paragraph). Maintainer inputs, 2026-09-16: the M1 browser-on-real-data gap is folded into M2 (a breeder takes real segregating data to selected.csv in the browser); the Pages site stays public for now; M2 acceptance uses real data for the two-generation round trip; no cM map exists. Fable research, 2026-09-16, pending maintainer confirmation:
     - Downstream consumer: nothing in isoline-browser reads this tool's outputs. Recommended a documented R reader (`scripts/read_results.R`, readr) instead of a dashboard, plus a results.csv header test. Before freezing columns: add `background_model`, `rpp_unit`, `rank_mode` and `results_schema` (SemVer); route `het_rate`, `expected_het` and `expected_rpp` through `_num` (today a literal `nan` can be written); write the header row when there are no results. Open: empty cells or `NA` for missing values (isoline-browser writes `NA`); whether R enters CI.
     - Composite default: keep weighted; add `ranking: {mode: weighted|staged}` with staged order recombinant flanks, rpp_carrier, rpp_noncarrier, drag, missing_rate, sample_id; amend ADR 0007. Frisch et al. 1999's staged advantage is genotyping cost under sequential genotyping, which does not apply to an already-genotyped file.
     - Real two-generation data: none confirmed public. Best candidate: rice Samgwang x Milkyqueen MABC, Kim et al. 2021 Plants 10:804 (CC BY; KASP; BC1F1 n=210, BC2F1 n=196; Table S3 may hold per-plant calls, MDPI blocks fetchers). Alternates are single-generation: rice WTR-1 BC1F5 tGBS (Harvard Dataverse doi:10.7910/DVN/RRXCR3, CC0 vs CC BY-NC terms unclear), tomato S. pennellii BILs (Dryad, CC0, has cM). Any rice or tomato data needs non-soybean chromosome names (a contract change starting in isoline-browser).
     - Platforms and assemblies: soybean public data is SoySNP50K (gnm1, gnm2) and BARCSoySNP6K (Wm82.a2). SoyBase Data Store has per-assembly marker GFF3s (`Wm82.gnm{1,2,4,5,6}.mrk.SoySNP50K`, `.SoySNP6K`, `.consensus40`; license Open) that can be joined on marker id into a position table across assemblies. `core/chrom.py` hardcodes Wm82.a4 lengths although a2 lengths exist in `constants.py`; make the assembly selectable.
     - cM map: Song et al. 2016 BMC Genomics 17:33 Table S1 (CC BY 4.0; 27,431 SoySNP50K SNPs, cM on two maps, Glyma1.01 and Wm82.a2 positions). Interpolate unmapped markers from bp against map anchors on the same assembly (Marey map, monotone, clamped).
     - Maintainer answers, 2026-09-16:
       - There is no external R dashboard. "the program's R Shiny dashboards" (Workflow target, the architecture flowchart node, and the M2 acceptance "consumed by a downstream Shiny dashboard") was an assumption written in the initial scaffold (4c7d808, 2026-09-09). The M2 acceptance for the frozen CSV columns needs rewording at M2 planning; ask the maintainer what should consume results.csv.
       - Missing cells in results.csv are written `NA`, matching isoline-browser. Freeze with that.
       - Kim et al. 2021: the supplement was fetched through the Europe PMC API (PMC's own link is behind a bot check; MDPI returns 403) into gitignored `data/kim2021/`. Table S3 lists the 96 background KASP markers with positions and the two parents' A/B calls only. There are no per-plant progeny calls and no pedigree, so it cannot serve the two-generation acceptance. The maintainer will find a fallback.
       - Non-soybean crops (rice, tomato; contract 1.3.0 crop profiles) move to M3.
       - ADR 0007 amended 2026-09-16: `ranking.mode` weighted (default) or staged, to be implemented in M2.
...
- Coordination: contract 1.2.0 mirrored and the loaders aligned (docs/adr/0010; isoline-browser docs/adr/0014). Any further contract change starts in isoline-browser and is mirrored here by copying `contract/`; `python scripts/check_contract.py ../isoline-browser` proves the copy. Undecided in 1.1.0: half-missing pairs (`AN`, `A-`), read as missing. Contract 1.3.0 (after M3): named token profiles under `contract/profiles/` and a crop selector, designed in isoline-browser PLAN.md and docs/adr/0014.
