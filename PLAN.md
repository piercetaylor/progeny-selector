...
- Model for next phase: phase 7 is UI (Selection notes, the Validate map/model block and duplicates, Export per-selected, the keyboard walkthrough) and goes to an `implementer` on sonnet with no reviewer pass, per `docs/m2-phases.md`; phase 8 can run beside it on disjoint files once the maintainer supplies the two-generation dataset. Implementers per CLAUDE.md thereafter (opus for core, io, model, contract and docs/data-formats.md).
