...
- Model for next phase: phase 6 goes to an `implementer` on opus (it touches core and the fixture generator) with a `reviewer` pass on the diff; implementers per CLAUDE.md thereafter (opus for core, io, model, contract and docs/data-formats.md).
...
- Gate baseline, 2026-09-21 at 2b966bc: 371 unit tests pass, 19 deselected; 16 browser tests plus 1 export smoke test (skipped without PS_SITE_DIR), counts from M1; site 44.4 MB, Pyodide ready in 8.7 s locally. Browser waits default to 30 s (PS_E2E_TIMEOUT_MS).
