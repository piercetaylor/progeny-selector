...
- Model for next phase: phase 7 is UI (Selection notes, the Validate map/model block and duplicates, Export per-selected, the keyboard walkthrough) and goes to an `implementer` on sonnet with no reviewer pass, per `docs/m2-phases.md`. Phase 8, the two-generation *synthetic* round trip, depends only on phases 4 and 6 and is dispatchable now: its precondition Q13 is answered and it needs no maintainer dataset, since Question B blocks the real-data step in phase 11, not this one. The order table pairs 7 with 8 on disjoint files, so both can run at once; the second to finish puts its CHANGELOG line in its report for the main session to insert. Implementers per CLAUDE.md thereafter (opus for core, io, model, contract and docs/data-formats.md).
- Review required: on any diff touching core, io, model or contract paths; one pass at the end of M2.
- Open maintainer questions: B (two-generation dataset); whether `validate` should report pairs skipped for insufficient overlap, since ADR 0017's amendment records that this false negative is currently silent (raised 2026-09-21, not urgent: the flag is advisory); and whatever `docs/m2-phases.md` flags. Known limits deferred from M1 (`docs/m1-phases.md`, phase 4 section): no "(no generation)" radio choice; Rank caption omits an unassigned family; the Navigate stale-input guard has no browser test.
...
- Gate baseline, 2026-09-21 at 66f968c: 384 unit tests pass, 19 deselected; 16 browser tests plus 1 export smoke test (skipped without PS_SITE_DIR), counts from M1; site 44.4 MB, Pyodide ready in 8.7 s locally. Browser waits default to 30 s (PS_E2E_TIMEOUT_MS).
