...
- Milestone / phase: M2 in progress from `docs/m2-phases.md`. Done: phase 1 (deferred core/io findings, 0bb7712), phase 2 (deferred UI/CI/test findings, b8382f2), phase 9 (genetic map interpolation and the Song 2016 converter, 19a9cfc), phase 3 (ranking.mode staged, the assembly selector and map warnings, 19c2770, docs/adr/0015). Also mirrored since M1: contract 1.3.0 (96a2114, docs/adr/0013) and 1.4.0 token profiles (17464e3 with review fixes b2c347c, docs/adr/0014). Next: phase 4 (results.csv freeze), then 5, 6, 7, 8, 10, 11. Last commit 19c2770 plus this handoff line; pushed yes; CI green; Pages public at https://piercetaylor.github.io/progeny-selector/. Gate baseline: 360 unit tests. Wm82.a5 and a6 lengths are still unverified and deliberately absent from ASSEMBLIES.
- Uncommitted in the working tree, 2026-09-16, from another session: the contract 1.3.0 mirror (blank lines, `#` as data, quoted line breaks, `genotypes.column_count`; docs/adr/0013, backcross docs/adr/0018) with its io reader, test, CHANGELOG.md and docs/data-formats.md edits. The matching canonical edits are uncommitted in the sibling. Leave them to that session; stage by name.
- Uncommitted, 2026-09-16: contract 1.4.0 mirrored (token profiles; docs/adr/0014, backcross docs/adr/0019 with its review amendment): `io/profiles.py`, profile paths in calls, hapmap, wide_csv and `load_dataset`, `Dataset.token_profile`, `--profile` on validate and rank, the Load screen's Token profile select and clearable custom file, `token_profile` last in results.csv and selected.csv, `tests/test_profiles.py`; gates pass and `check_contract.py ../isoline-browser` reports the mirror identical. Canonical edits are uncommitted in the sibling; commit both together.
- Next:
...
- Model for next phase: planner (fable) for M2; implementers per CLAUDE.md (opus for core, io, model, contract and docs/data-formats.md).
- Review required: on any diff touching core, io, model or contract paths; one pass at the end of M2.
- Open maintainer questions: B (two-generation dataset), and whatever `docs/m2-phases.md` flags. Known limits deferred from M1 (`docs/m1-phases.md`, phase 4 section): no "(no generation)" radio choice; Rank caption omits an unassigned family; the Navigate stale-input guard has no browser test.
...
- Coordination: the sibling is `backcross` (GitHub piercetaylor/backcross, package name backcross), but its folder on disk is still `../isoline-browser`, so `python3 scripts/check_contract.py ../isoline-browser` is the working form until the maintainer renames the folder (CLAUDE.md already says `../backcross`). On 2026-09-16 it reported 159 files identical, both trees at uncommitted 1.3.0. Any contract change starts in backcross and is mirrored here by copying `contract/`. Undecided since 1.1.0: half-missing pairs (`AN`, `A-`), read as missing.
- Gate baseline: 205 unit tests (17 deselected); 16 browser tests plus 1 export smoke test (skipped without PS_SITE_DIR), counts from M1; site 44.4 MB, Pyodide ready in 8.7 s locally. Browser waits default to 30 s (PS_E2E_TIMEOUT_MS).
