...
- Milestone / phase: **Contract 1.5.0 is complete in both repositories**, 2026-09-22. Backcross phase 6 is `d4ad0e8`; this repository's S6 is `8ba8cab` and the assembly fix that followed it is `fc21019`. The mirror reports 245 files identical in both directions. Gates here at `fc21019`: 477 passed, 27 deselected; `pytest -m e2e` 26 passed, 1 skipped. CI not yet read against `fc21019`; `e2e`, `export` and `r-reader` are CI-only and the `r-reader` job is the authority on `scripts/read_results.R`, which this change touched.
