...
- Model for next phase: phase 11 is the main session with the maintainer — real-data acceptance in the browser locally and on Pages, the Question B step (still blocked on the dataset), the end-of-milestone reviewer pass over the whole M2 diff, and the docs. No implementer dispatch is called for until that pass produces findings.
