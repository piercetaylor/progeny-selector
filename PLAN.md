...
The crop work is not this milestone. What M2 deferred is the crop chromosome schemes, and not the token profiles, which landed in M2 as contract 1.4.0. The schemes are contract 1.5.0, specified in `../isoline-browser/docs/m4-phases.md` section 8 with this repository's half as S6. A contract version originates in backcross and is copied here, so S6 cannot precede backcross phase 6 and is not a design task for this repository. S6 lands before the M3 phases, because it threads a crop scheme through `io/vcf.py`, which the streaming parse then rewrites.
...
- Milestone / phase: **Contract 1.5.0 is complete in both repositories**, 2026-09-22. Backcross phase 6 is `d4ad0e8`; this repository's S6 is `8ba8cab` and the assembly fix that followed it is `fc21019`. The mirror reports 245 files identical in both directions. Gates here at `fc21019`: 477 passed, 27 deselected; `pytest -m e2e` 26 passed, 1 skipped. CI run 35785032182 green on all six jobs against `fc21019`, which is the run that matters: `e2e`, `export` and `r-reader` are CI-only, and `r-reader` is the authority on `scripts/read_results.R`, which the crop column and the assembly fix both touched. Run 35785063233 green against `b1753d1`.

  M3 is next and is the hardening list, decided 2026-09-22 and recorded above under "M3 scope". Nothing blocks the planner: S6 landed, which was the only dependency. The order research recommended is the inherited `next_generation` fix, then a measurement phase publishing a limits table, then streaming `read_vcf` on top of the S6 code with the accessibility and screen-test gaps in parallel on disjoint files, then the BrAPI loader. `donor_parent_2` is out, settled by the maintainer on 2026-09-22.
