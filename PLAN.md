...
- Milestone / phase: **M3 phases 1-4 are done**, 2026-09-23, at `25852a2`. Contract 1.5.0 remains complete in both repositories, 2026-09-22. Backcross phase 6 is `d4ad0e8`; this repository's S6 is `8ba8cab` and the assembly fix that followed it is `fc21019`. The mirror reports 245 files identical in both directions. Gates here at `fc21019`: 477 passed, 27 deselected; `pytest -m e2e` 26 passed, 1 skipped. CI run 35785032182 green on all six jobs against `fc21019`, which is the run that matters: `e2e`, `export` and `r-reader` are CI-only, and `r-reader` is the authority on `scripts/read_results.R`, which the crop column and the assembly fix both touched. Run 35785063233 green against `b1753d1`.

  M3 is in progress against `docs/m3-phases.md`, whose nine phases were specified on 2026-09-22 and whose 17 questions are answered in that file. Phase 8 is struck from this milestone: BrAPI is command line only in M3. Done: phase 1 `next_generation` validation (`08e5021`), phase 2 the measurement harness and `docs/limits.md` (`c3e54ab`), phase 4 the screen-test gaps (`59b1170`), phase 3 streaming `read_vcf` (`25852a2`). Gates at `25852a2`: 505 passed, 30 deselected; `pytest -m e2e` 29 passed, 1 skipped; contract mirror 245 files identical; regenerated fixture byte-identical. Remaining: phase 5 accessibility, phase 6 the analysis-memory bound, phase 7 the BrAPI loader, phase 9 re-measurement and this block. The sibling folder was renamed from `isoline-browser` to `backcross` on 2026-09-23, so `scripts/check_contract.py` takes `../backcross`.

  Measurements still owed to `docs/limits.md`, held on hardware not on a decision: the `50k_x_2000` CPython row and all three Shinylive rows, with the Chromium version for the header. The reference machine has 15.4 GB of RAM and the case needs about 4 GB of available physical memory to measure `core` rather than the pagefile; it had 0.5 GB available on 2026-09-23 with Firefox holding 3.4 GB. Phase 3 did not wait for it: the parser's own before-and-after was taken on the existing `50k_x_200` case by restoring `c3e54ab`'s `vcf.py` from history and measuring both parsers on the same file minutes apart, 68.4 MiB against 38.4 MiB. Analysis memory already has baselines at two shapes, about 18x the genotype matrix in both, so phase 6 is not blocked: its acceptance is an absolute threshold, the 50K x 2,000 analysis peak at or below 4x the resident matrix in phase 9's table, so it needs that row measured after the phase and no pre-phase baseline at all. What the missing baseline buys is only the before-and-after narrative in `docs/limits.md`, at the marker-heavy and progeny-heavy corner together, which is where an un-chunked `rpp` is worst.

  Next dispatch, per the order table: phases 6 and 7 together, the parallel pair, both opus and both reviewed; phase 5 depends only on phase 4 and can follow or run alongside on disjoint files, subject to the two-concurrent-implementer limit. Phase 9's brief must record the parse-peak ratio at both measured shapes, 2.0x at 50,000 x 202 and 1.21x at 2,000 x 1,000, so the regression test's 1.5x threshold is not read as a general bound.
