...
M3 hardened: streaming VCF parse to bound memory; measured performance and a documented limits table for CPython and Shinylive; accessibility review; the screen-test gaps M1 deferred; BrAPI loader. Acceptance: contract tests pass in both repositories; the limits table records measured figures for 6K x 2,000, 50K x 200 and 50K x 2,000 on the reference laptop, and for the first two in Shinylive.
...
- Milestone / phase: **M3 phases 1-4, 6 and 7 are done**, 2026-09-23, at `4d93907`. Contract 1.6.0 is committed in both repositories; its mirror commit here is `9f0cbd6`, made by the backcross session, and `scripts/check_contract.py ../backcross` reports 259 files identical in both directions. Gates at `4d93907`: 559 passed, 30 deselected; the regenerated fixture is byte-identical. CI run 35922180289 is green on all six jobs against `4d93907`, including the CI-only `check (3.11)`, `e2e`, `export` and `r-reader`. `pytest -m e2e` was not re-run locally; its last local figure is 29 passed, 1 skipped at `25852a2`.

  M3 is in progress against `docs/m3-phases.md`, whose nine phases were specified on 2026-09-22 and whose 17 questions are answered in that file. Phase 8 is struck from this milestone: BrAPI is command line only in M3. Done: phase 1 `next_generation` validation (`08e5021`), phase 2 the measurement harness and `docs/limits.md` (`c3e54ab`), phase 3 streaming `read_vcf` (`25852a2`), phase 4 the screen-test gaps (`59b1170`), phase 6 the analysis-memory bound (`b2bd711`, docs/adr/0025) and phase 7 the BrAPI loader (`4d93907`, docs/adr/0024). Remaining: phase 5 accessibility, phase 9 re-measurement and the end-of-milestone review, and this block. The sibling folder was renamed from `isoline-browser` to `backcross` on 2026-09-23; the phase 7 spec still says `../isoline-browser` and means `../backcross`.

  Phase 6 as shipped. Weighted `rpp` was not bit-identical when the sample count left a one-column final block, at 65, 129, 193 or 2,049 columns (`n_s % 64 == 1`). numpy reduces a single-column array by pairwise summation and a wider one by accumulating rows in order, which moved the last individual's RPP by up to 4.7e-15 at 50,000 markers. The reviewer found it; the original tests at 700 and 40 samples could not. `constants.sample_blocks` folds a width-1 tail into the preceding block, and the boundary tests cover 64, 65, 128, 129, 193, 700 and 2,049 samples. A sweep of remainders 0 to 8 at 50,000 markers failed at remainder 1 only. The first draft also let one infinite marker weight turn every sample's RPP denominator to NaN; the pre-phase `np.where` form is restored and tested. Block temporaries depend on the marker count alone: 83.2 MB for `rpp` at 50,000 markers, measured unchanged between 100 and 2,000 individuals. A ratio against the matrix is therefore shape-dependent, 16.65x at 4,000 x 100 and 1.67x at 4,000 x 1,000, and prose should quote megabytes with a marker count. `rpp_contribution` in `core/classify.py` is now unused in `src/` and holds a second copy of the 1/0.5/0 table; ADR 0025 records its removal as a follow-up.

  Phase 7 as shipped. The reviewer found two silent-corruption paths and one credential leak in the first draft, all fixed and tested. A repeated `/allelematrix` page was detected only when every cell was already written, so a single missing call defeated the check; the guard now compares page identity, the variant and call-set id lists. Variants and call sets that no page covered are warned about, as the sibling does. `urllib` forwarded the bearer token through a redirect to another host, and a redirect handler now drops it. `BRAPI_MAX_CALLS` is `6_000 * 2_002`, the exact call count of the `6k_x_2000` row, so the largest completed case does not warn. Two differences from backcross are deliberate and recorded in ADR 0024. The allele pair keeps server order, which no consumer distinguishes. A half-call reads `(-1, -1)` where `read_vcf` gives `(-1, 1)`, with identical results, since every consumer tests either allele below zero.

  Working alongside the backcross session. On 2026-09-23 a separate session working contract 1.6.0 in backcross mirrored `contract/`, `io/calls.py`, `tests/test_io.py`, `docs/data-formats.md`, `CHANGELOG.md` and `PLAN.md` into this working tree while phases 6 and 7 were uncommitted. The changes were first taken for a phase 7 implementer overreaching; file modification times and `ListAgents` showed the live sibling session, and the implementer's own account was accurate. The collision was settled by agreement. That session committed its files in `9f0cbd6`, taking `docs/data-formats.md` whole with phase 7's BrAPI section and saying so in its commit body, and phases 6 and 7 were committed afterwards. Commit only by explicit pathspec, never `git add -A`. Treat a gate failure in a file outside the phase's list as another writer's mid-edit state and re-run before reporting it. Edit a shared file such as `CHANGELOG.md` only after the other writer's hunk has landed, so each commit carries its own entry only.

  Measurements still owed to `docs/limits.md`, held on hardware and not on a decision: the `50k_x_2000` CPython row and all three Shinylive rows, with the Chromium version for the header. The case needs about 4 GB of available physical memory to measure `core` and not the pagefile. On 2026-09-23 the reference machine had 0.83 GB available of 15.4 GB, with Firefox holding 3.46 GB across 19 processes. `scripts/bench_pipeline.py` checks free disk and does not check RAM, so check `(Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory / 1MB` before launching. The run takes about 2 hours and peaks near 3.6 GB; run it in the background, and record "did not complete" if it does not (question 16). This row is now binding, because phase 6's acceptance is the 50K x 2,000 analysis peak at or below 4x the resident matrix, judged in phase 9's table. Phase 3's parse before-and-after was taken on `50k_x_200` and needs nothing further.

  Next dispatch: phase 5, accessibility, sonnet, gates only unless its diff reaches core, io, model or contract paths. Phase 9 follows once the owed row is measured. Its brief must record the parse-peak ratio at both measured shapes, 2.0x at 50,000 x 202 and 1.21x at 2,000 x 1,000, so the regression test's 1.5x threshold is not read as a general bound. It must state phase 6's bound in megabytes at a marker count for the same reason. The end-of-milestone reviewer pass covers the whole M3 diff, from the parent of `08e5021` to HEAD.

  Open across the two repositories, none blocking, all originating in `backcross` where the contract is canonical. Python `.strip()` keeps U+FEFF where JavaScript `.trim()` removes it, so a cell holding that character reads as one character here and as empty in the sibling; `open_text` strips a BOM at file start, so it bites only inside a quoted cell mid-file, and the fix belongs in the shared readers. The sibling's `docs/reference-repos.md` says the former `plantbreeding/API` paths return 404; they redirect, because the repository was renamed to `plantbreeding/BrAPI`, and this repository's entry was corrected to the canonical name. The sibling's `docs/data-formats.md:9` heading "Backcross only" becomes false once phase 7 lands a BrAPI loader here.

  Pre-existing in `read_vcf`, unchanged by phase 3 and identical in the pre-phase parser, found by the phase 3 reviewer and deliberately left alone: a GT allele index of 128 or more raises a raw `OverflowError` from numpy before the ALT-count check can report it, and `int()` leniency accepts `-5/0` as -5, `1_0/0` as 10 and `" 1/0"` as 1. A Latin-1 VCF still escapes as a raw `UnicodeDecodeError`. Each is a contract-error candidate for a later milestone.

  Carried forward from contract 1.5.0, none blocking: the Load screen's `crop=` wiring and Compare's `dataset.scheme` argument have no failing test, by the same precedent as the existing `profile=` wiring; `background.marker_weights` takes a scheme that affects only iteration order, verified to have no numerical effect, so it has no discriminator; an unknown `--crop` exits 1 through the top-level catch at `cli.py` while other usage errors exit 2, and an unknown `--profile` does the same, so the two are at least consistent. The `assembly` key now names three different things in this system: the criteria key and results column, which are the length table the run used, and the crop scheme's own `assembly` field, which is free-text naming provenance. That collision is recorded here and in `docs/adr/0015`'s 2026-09-22 amendment; renaming either would be a contract change.
