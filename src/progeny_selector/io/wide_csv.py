...
Responsibility: parse nucleotide calls ("A", "AT", "A/T") or A/B/H coding with
auto-detection; when coded, the parents may be absent from the file and are
synthesised as all-A (recurrent) and all-B (donor) if the caller asks.
...
def read_wide_csv(path: str | Path, coding: str = "auto") -> GenotypeMatrix:
    ...
    if coding == "auto":
        coding = detect_coding(cell for row in records[: min(len(records), 200)] for cell in row[3:])
    markers: list[Marker] = []
